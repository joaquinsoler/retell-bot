...
SESIONES_ACTIVAS = {}   # IP → {"email": str, "expira": datetime}
...
- Dar información detallada del negocio y agendar nuevas citas.
- Si te piden cancelar o modificar una cita, responde educadamente que solo puedes agendar nuevas.
            ...
            "htmlContent": f"""
                <html><body style="font-family:sans-serif;padding:30px;background:#f8fafc;">
                    <div style="max-width:500px;margin:auto;background:white;padding:30px;border-radius:16px;">
                        <h2>Accede a tu Panel Dansu</h2>
                        <p>Haz clic en el botón para entrar directamente:</p>
                        <a href="{magic_link}" style="background:#0078FF;color:white;padding:14px 28px;text-decoration:none;border-radius:12px;font-weight:600;display:inline-block;">
                            Entrar al Panel ✨
                        </a>
                    </div>
                </body></html>
...
async def redirect_to_wix(token: str, request: Request):
    ...
    <body><h3>Verificación exitosa. Redirigiendo...</h3></body></html>
    """)
...
async def update_retell_bot_endpoint(request: Request):
    try:
        ...
        """, (data.get("nombre_negocio"), data.get("sector"), data.get("servicios"),
              ...
              data.get("asistente"), data.get("agent_id")))
...
async def delete_retell_bot_endpoint(request: Request):
    try:
        ...
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("DELETE FROM asistentes WHERE agent_id = %s;", (data.get("agent_id"),))
        conn.commit()
        cur.close()
        conn.close()
...
@app.get("/")
async def root():
    return {"status": "✅ Dansu Backend Completo - Magic Link + IP Session Activo"}
