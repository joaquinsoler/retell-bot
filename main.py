...
async def update_retell_bot_endpoint(request: Request):
    ...
    prompt reforzado y la voz con la API de Retell AI en vivo de manera compatible.
    """
    try:
        data = await request.json()
        ...
        asistente_nombre = data.get("asistente") # Nombre comercial de la voz (ej: "Cimo")
        ...
        # 4. Actualización de voz dinámica en Retell AI
        ...
        if voice_id_tecnico:
            ...
            voice_id_tecnico = agent_info.get("voice_id") # Mantiene la voz si no se envió mapeo correcto

        # 5. Actualizar el registro de manera persistente en PostgreSQL
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("""
            UPDATE asistentes 
...
async def delete_retell_bot_endpoint(request: Request):
    ...
    Incluye un sistema tolerante a fallos para limpiar la BD si el bot ya fue borrado en Retell.
    """
    try:
        data = await request.json()
        agent_id = data.get("agent_id")
        ...
        # 1. Intentar obtener detalles en Retell AI para desvinculaciones
        agent_info = retell_request("GET", f"/get-agent/{agent_id}")
        
        if agent_info and isinstance(agent_info, dict):
            llm_id = agent_info.get("response_engine", {}).get("llm_id")
            
            # 2. Buscar si el número de teléfono está asignado y liberarlo
            try:
                ...
                print(f"⚠️ No se pudo liberar el teléfono (posiblemente libre o error de API): {e_phone}")

            # 3. Eliminar el Agente de Retell AI
            retell_request("DELETE", f"/delete-agent/{agent_id}")
            
            # 4. Eliminar el motor LLM asociado
            if llm_id:
        ...
        # Failsafe de última línea para limpiar la pantalla del usuario pase lo que pase
        try:
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute("DELETE FROM asistentes WHERE agent_id = %s;", (agent_id,))
