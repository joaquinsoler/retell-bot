...
app = FastAPI(title="Dansu Backend Completo con Magic Link - Corregido")
...
def normalize_to_madrid_iso(dt_str: str) -> str:
    """
    Normaliza cualquier string de fecha/hora que el LLM pueda enviar a formato ISO 8601
    con offset de Europe/Madrid. Muy robusto contra variaciones comunes del LLM.
    """
    if not dt_str:
        return dt_str
    ...
    # Intento 1: fromisoformat directo (el más común y limpio)
    try:
        if cleaned.endswith("Z"):
            base = cleaned[:-1]
            dt = datetime.fromisoformat(base).replace(tzinfo=ZoneInfo("UTC"))
        else:
    ...
    # Intento 2: Añadir segundos si faltan (error MUY común del LLM: 2026-07-15T10:00)
    try:
        if "T" in cleaned and cleaned.count(":") == 1:
            # Ej: 2026-07-15T10:00  -> 2026-07-15T10:00:00
            cleaned2 = cleaned + ":00"
    ...
    # Intento 3: strptime con formatos comunes que usa el LLM
    for fmt in [
        "%Y-%m-%dT%H:%M:%S",
        "%Y-%m-%dT%H:%M",
        "%Y-%m-%d %H:%M:%S",
    ...
    # Último recurso: devolver versión limpia (el error se detectará después en el insert)
    print(f"⚠️ WARNING normalize_to_madrid_iso: No se pudo parsear '{original}'. LLM debería usar ISO 8601.")
    return cleaned
...
def create_google_event(calendar_id: str, summary: str, start_time: str, end_time: str, description: str = "", bypass_availability: bool = False):
    try:
        ...
        # Validación básica de orden temporal
        try:
            ...
            pass  # Si falla el parseo aquí, lo atrapará después
...
**FORMATO ESTRICTO PARA book_appointment (OBLIGATORIO - MUY IMPORTANTE):**
Cuando uses la herramienta `book_appointment`, los parámetros `start_time` y `end_time` DEBEN ser SIEMPRE strings en formato ISO 8601 completo con segundos y offset de Madrid.
Ejemplo correcto: "2026-07-15T10:30:00+02:00"
- NUNCA uses lenguaje natural ("mañana a las 10", "el jueves por la tarde", etc.).
- NUNCA omitas la fecha completa, los segundos ni el offset.
- Calcula `end_time` sumando duración razonable según el servicio (30 minutos para la mayoría de servicios, 45-60 min si es más largo).
Ejemplo: si el cliente quiere el 15 de julio a las 10:00 → start_time="2026-07-15T10:00:00+02:00", end_time="2026-07-15T10:30:00+02:00"
...
1. **Día y Hora:** Propón o confirma el momento de la cita según las preferencias del cliente. Usa SIEMPRE el formato ISO correcto cuando llames a la herramienta.
...
- Si la herramienta `book_appointment` te devuelve un mensaje que contiene "SLOT_OCUPADO" o indica que el hueco está ocupado, actúa como un comercial humano resolutivo y amable:
...
- Si la herramienta devuelve un mensaje que contiene "ERROR_FORMATO_HORA", responde de forma muy amable pidiendo que concrete la fecha y hora:
  *"Disculpa, parece que no capté bien la fecha y la hora exacta. ¿Me puedes indicar el día y la hora precisa que prefieres? Por ejemplo: el martes 15 a las 10 de la mañana."*
...
def create_bot_for_client(nombre_negocio, sector, servicios, horario, zona, voice_id, calendar_email):
    ...
    tool_definition = {
        ...
        "description": "Agenda una nueva cita en el calendario del negocio. Usa SIEMPRE start_time y end_time en formato ISO 8601 exacto con offset de Madrid (ej: 2026-07-15T10:30:00+02:00). Devuelve error si el hueco está ocupado o hay problema de formato.",
        ...
        "parameters": {
            ...
            "properties": {
                "calendar_email": {
                    "type": "string",
                    "description": "Email exacto del Google Calendar del negocio (obligatorio)"
                },
                "summary": {
                    "type": "string",
                    "description": "Título de la cita, normalmente 'Cita con [Nombre Cliente] - [Servicio]'"
                },
                "start_time": {
                    "type": "string",
                    "description": "Fecha/hora INICIO en ISO 8601 con offset Madrid. Ejemplo: '2026-07-15T10:00:00+02:00'. OBLIGATORIO este formato exacto."
                },
                "end_time": {
                    "type": "string",
                    "description": "Fecha/hora FIN en ISO 8601 con offset Madrid. Ejemplo: '2026-07-15T10:30:00+02:00'"
                },
                "description": {
                    "type": "string",
                    "description": "Notas adicionales o motivo de la cita (opcional)"
                }
            },
...
async def update_retell_bot_endpoint(request: Request):
    try:
        ...
        # Tool definition mejorada (igual que en creación)
        tool_definition = {
            "type": "custom",
            "name": "book_appointment",
            "description": "Agenda una nueva cita en el calendario del negocio. Usa SIEMPRE start_time y end_time en formato ISO 8601 exacto con offset de Madrid (ej: 2026-07-15T10:30:00+02:00). Devuelve error si el hueco está ocupado o hay problema de formato.",
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "calendar_email": {"type": "string", "description": "Email exacto del Google Calendar del negocio (obligatorio)"},
                    "summary": {"type": "string", "description": "Título de la cita, normalmente 'Cita con [Nombre Cliente] - [Servicio]'"},
                    "start_time": {"type": "string", "description": "Fecha/hora INICIO en ISO 8601 con offset Madrid. Ejemplo: '2026-07-15T10:00:00+02:00'. OBLIGATORIO este formato exacto."},
                    "end_time": {"type": "string", "description": "Fecha/hora FIN en ISO 8601 con offset Madrid. Ejemplo: '2026-07-15T10:30:00+02:00'"},
                    "description": {"type": "string", "description": "Notas adicionales o motivo de la cita (opcional)"}
                },
                "required": ["calendar_email", "summary", "start_time", "end_time"]
            }
        }
...
async def book_appointment(request: Request):
    try:
        ...
        print(f"📥 Tool call book-appointment recibido - args: {args}")
        ...
        if "ya no está disponible" in error_lower or "ocupado" in error_lower or "busy" in error_lower:
            return {"code": "ERROR", "message": "SLOT_OCUPADO: El horario seleccionado ya no está disponible."}
        elif any(x in error_lower for x in ["datetime", "format", "iso", "parse", "invalid time", "time"]):
            return {"code": "ERROR", "message": "ERROR_FORMATO_HORA: Formato de fecha/hora no válido. Por favor propone una fecha y hora en formato ISO 8601 (ej: 2026-07-15T10:00:00+02:00)."}
        else:
...
@app.get("/")
async def root():
    return {"status": "Dansu Backend Completo OK - Corregido con parsing robusto de datetime y manejo inteligente de errores"}
